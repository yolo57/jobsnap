...
export function createBrowserClient(token) {
  return {
    from: (table) => new RestQuery(SUPABASE_URL, table, token || SUPABASE_ANON_KEY, SUPABASE_ANON_KEY),
  };
}
...
class RestQuery {
  constructor(url, table, authToken, apiKey) {
    this.url = `${url}/rest/v1/${table}`;
    // authToken identifies the caller (user JWT, or an api key when no session) for RLS.
    // apiKey is always a real Supabase project key, required by the gateway regardless of who's calling.
    this.authToken = authToken;
    this.apiKey = apiKey || authToken;
    this.filters = [];
    this.selectCols = '*';
    this.orderCol = null;
    this.limitNum = null;
  ...
  then(resolve, reject) {
    ...
    else if (this.upsertData) { method = 'POST'; body = this.upsertData; this.url += (this.url.includes('?') ? '&' : '?') + 'on_conflict=id'; }
...
export const supabase = {
  ...
  from: (table) => {
    try {
      ...
      return new RestQuery(process.env.NEXT_PUBLIC_SUPABASE_URL, table, token || SUPABASE_ANON_KEY, SUPABASE_ANON_KEY);
    } catch {
      return new RestQuery(process.env.NEXT_PUBLIC_SUPABASE_URL, table, SUPABASE_ANON_KEY, SUPABASE_ANON_KEY);
    }
  },
};
